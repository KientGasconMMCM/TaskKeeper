...
const sendResetEmail = async (toEmail, resetToken, baseUrl) => {
  ...
  const mailOptions = {
    ...
    to: toEmail,
    subject: 'CramTaskGPT - Password Reset',
    html: `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 30px; background: #1a2332; border-radius: 8px; color: #fff;">
        <h1 style="color: #0896ee; text-align: center; margin-bottom: 20px;">CramTaskGPT</h1>
...
const sendTaskCreatedEmail = async (toEmail, task) => {
  ...
  const mailOptions = {
    from: process.env.SMTP_FROM || process.env.SMTP_USER,
    to: toEmail,
    subject: 'CramTaskGPT - Created Task!',
    html: `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 30px; background: #1a2332; border-radius: 8px; color: #fff;">
        <h1 style="color: #0896ee; text-align: center; margin-bottom: 20px;">CramTaskGPT</h1>
        ...
        <p style="color: #999; font-size: 13px; text-align: center;">Stay on top of your tasks with CramTaskGPT!</p>
      </div>
    `,
  };
...
const sendDeadlineReminderEmail = async (toEmail, task) => {
  ...
  const mailOptions = {
    from: process.env.SMTP_FROM || process.env.SMTP_USER,
    to: toEmail,
    subject: 'DEADLINE REMINDER FOR CRAMTASKGPT!',
    html: `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 30px; background: #1a2332; border-radius: 8px; color: #fff;">
        <h1 style="color: #0896ee; text-align: center; margin-bottom: 20px;">CramTaskGPT</h1>
...
const sendWelcomeEmail = async (toEmail, username) => {
  const mailOptions = {
    from: process.env.SMTP_FROM || process.env.SMTP_USER,
    to: toEmail,
    subject: 'Welcome to CramTaskGPT!',
    html: `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 30px; background: #1a2332; border-radius: 8px; color: #fff;">
        <h1 style="color: #0896ee; text-align: center; margin-bottom: 20px;">CramTaskGPT</h1>
        ...
        <p style="color: #999; font-size: 13px; text-align: center;">Thank you for choosing CramTaskGPT!</p>
      </div>
    `,
  };
